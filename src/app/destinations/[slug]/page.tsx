...
export default function DestinationDetailPage() {
  ...
  return (
    <main ref={containerRef} className="min-h-screen bg-white overflow-hidden pb-24 font-montserrat">
        ...
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24">
          ...
          <div className="space-y-10 md:space-y-12">
            ...
            <motion.div 
               ...
               viewport={{ once: true }}
               className="p-8 md:p-12 bg-dark rounded-[2.5rem] md:rounded-[4rem] text-white relative overflow-hidden group shadow-2xl"
            >
               <div className="relative z-10 text-center md:text-left">
                 <h3 className="text-2xl md:text-3xl font-heading font-black mb-4 italic">Ready to Apply?</h3>
                 <p className="text-white/60 mb-8 font-medium text-sm md:text-lg">Get a personalized consultation with our country experts.</p>
                 <Link href={`/contact?country=${data.name}`} className="inline-flex items-center gap-4 bg-primary px-8 py-4 rounded-xl md:rounded-2xl font-black hover:bg-white hover:text-dark transition-all shadow-lg text-sm md:text-base">
                    Book Appointment →
                 </Link>
               </div>
      ...
      >
        <Link 
          href={`/contact?country=${data.name}`} 
          className="pointer-events-auto flex items-center justify-between md:justify-center gap-4 bg-dark text-white pl-8 pr-4 py-4 md:px-10 md:py-6 rounded-full shadow-2xl border border-white/10 hover:bg-primary transition-all group w-full md:w-auto"
        >
